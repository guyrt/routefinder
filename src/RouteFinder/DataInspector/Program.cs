...
static async Task DownloadAndDecodeAsync<T>(DataDownloadWrapper rawDataDownloader, string remoteFile, string localFile, Func<FileStream, T> parser)
    where T : IMessage<T>
{
...
static async Task DownloadAndDecodeAsync2(DataDownloadWrapper rawDataDownloader, string remoteFile, string localFile, Func<FileStream, FullWaySet> parser)
{
    Console.WriteLine($"Downloading {remoteFile} to {localFile}");
    await rawDataDownloader.RetrieveBlobAsync(remoteFile, localFile, overwrite: true);
